...
(define-non-fungible-token stacks-svg-nft uint)
...
(define-read-only (get-owner (id uint))
    (ok (nft-get-owner? stacks-svg-nft id)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
    (let ((owner (unwrap! (nft-get-owner? stacks-svg-nft id) ERR-NOT-FOUND))
...
(define-private (mint-many-iter (new-owner principal) (next-id uint))
    (if (or (is-eq MAX-SUPPLY u0) (<= next-id MAX-SUPPLY))
        (begin
            (unwrap! (nft-mint? stacks-svg-nft next-id new-owner) next-id)
            (+ next-id u1)
        )
        next-id))
